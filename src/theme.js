// theme.js — アプリ全体の配色・書体定義
//
// 実装方式: theme オブジェクトの各値は "var(--cs-xxx)" という文字列(CSS変数参照)。
// コンポーネント側は今まで通り theme.accent 等をそのまま使えば良く、
// 実際の色はブラウザ側の CSS変数(document.documentElement の --cs-*)から解決される。
// これにより、既存の数百箇所の theme.xxx 参照を一切書き換えずに
// 実行時のテーマ切替(色・書体)を実現している。

// ---- 色プリセット ----------------------------------------------------
export const THEME_PRESETS = {
  sky: {
    label: "空色（デフォルト）",
    bgFrom: "#EEF6FA", bgVia: "#DBEBF4", bgTo: "#C3DEED",
    text: "#0E344F", textSub: "#3D6A8E", textFaint: "#7AA3C2",
    surface: "#FFFFFF", surfaceAlt: "#EDF5FB", surfaceWhite: "#FBFDFF",
    border: "#A8CCE2", borderInput: "#8FBCD9", borderTable: "#CFE4F0",
    accent: "#0A5A8F", accentBright: "#1B7FBE", accentDeep: "#06436E",
    teamB: "#C4551C",
    faintAccent: "#96BEDA", faintAccent2: "#85AECD",
    headFrom: "#1B7FBE", headTo: "#0A5A8F", headBFrom: "#B44E18", headBTo: "#8C3A10", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#1B7FBE", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  red: {
    label: "紅",
    bgFrom: "#FAEFEF", bgVia: "#F4DCDC", bgTo: "#EEC7C7",
    text: "#4E0D12", textSub: "#903E44", textFaint: "#BF7C80",
    surface: "#FFFFFF", surfaceAlt: "#FBEFEF", surfaceWhite: "#FFFBFB",
    border: "#E2B0B3", borderInput: "#D99799", borderTable: "#F0D3D5",
    accent: "#7A1218", accentBright: "#A82026", accentDeep: "#570A0F",
    teamB: "#135F86",
    faintAccent: "#DC9DA0", faintAccent2: "#CF8C8F",
    headFrom: "#A82026", headTo: "#7A1218", headBFrom: "#135F86", headBTo: "#0E4868", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#A82026", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  green: {
    label: "若草",
    bgFrom: "#EFFAF0", bgVia: "#DCF4DF", bgTo: "#C7EECB",
    text: "#0D4E1F", textSub: "#3E9054", textFaint: "#7CBF8C",
    surface: "#FFFFFF", surfaceAlt: "#EFFBF1", surfaceWhite: "#FBFFFC",
    border: "#B0E2B8", borderInput: "#97D9A2", borderTable: "#D3F0D8",
    accent: "#127A2E", accentBright: "#1DA844", accentDeep: "#0A5720",
    teamB: "#C4551C",
    faintAccent: "#9DDCA8", faintAccent2: "#8CCF98",
    headFrom: "#1DA844", headTo: "#127A2E", headBFrom: "#B44E18", headBTo: "#8C3A10", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#1DA844", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  purple: {
    label: "藤",
    bgFrom: "#F4EFFA", bgVia: "#E6DCF4", bgTo: "#D5C7EE",
    text: "#2C0D4E", textSub: "#663E90", textFaint: "#9D7CBF",
    surface: "#FFFFFF", surfaceAlt: "#F5EFFB", surfaceWhite: "#FDFBFF",
    border: "#C7B0E2", borderInput: "#B597D9", borderTable: "#E0D3F0",
    accent: "#4E127A", accentBright: "#6D1DA8", accentDeep: "#390A57",
    teamB: "#BE5A22",
    faintAccent: "#B99DDC", faintAccent2: "#A88CCF",
    headFrom: "#6D1DA8", headTo: "#4E127A", headBFrom: "#B4571F", headBTo: "#8C4014", badgeBg: "transparent",
    profGreat: "#C93524", profGood: "#6D1DA8", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  yellow: {
    label: "山吹",
    bgFrom: "#FAF6EA", bgVia: "#F4EBD0", bgTo: "#EEDFB2",
    text: "#4E3A0D", textSub: "#907B3E", textFaint: "#BFAD7C",
    surface: "#FFFFFF", surfaceAlt: "#FBF7EC", surfaceWhite: "#FFFEF9",
    border: "#E2D2A0", borderInput: "#D9C488", borderTable: "#F0E6C8",
    accent: "#8F6A00", accentBright: "#B8890A", accentDeep: "#6B4F00",
    teamB: "#135F86",
    faintAccent: "#DCCB9D", faintAccent2: "#CFBC8C",
    headFrom: "#9C7A08", headTo: "#755B04", headBFrom: "#135F86", headBTo: "#0E4868", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#B8890A", profFair: "#C4551C", profWeak: "#9AA3AD",
  },
  orange: {
    label: "橙",
    bgFrom: "#FAF2EA", bgVia: "#F4E2D0", bgTo: "#EED1B2",
    text: "#4E290D", textSub: "#90603E", textFaint: "#BF9A7C",
    surface: "#FFFFFF", surfaceAlt: "#FBF4EC", surfaceWhite: "#FFFDF9",
    border: "#E2C4A0", borderInput: "#D9B188", borderTable: "#F0DEC8",
    accent: "#9C4A05", accentBright: "#C46312", accentDeep: "#753605",
    teamB: "#135F86",
    faintAccent: "#DCBB9D", faintAccent2: "#CFAA8C",
    headFrom: "#C46312", headTo: "#9C4A05", headBFrom: "#135F86", headBTo: "#0E4868", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#C46312", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  deepblue: {
    label: "濃紺",
    bgFrom: "#EEF3FA", bgVia: "#DCE6F4", bgTo: "#C7D8EE",
    text: "#0D2C4E", textSub: "#3E6690", textFaint: "#7C9DBF",
    surface: "#FFFFFF", surfaceAlt: "#EEF3FB", surfaceWhite: "#FCFDFF",
    border: "#B0C7E2", borderInput: "#97B5D9", borderTable: "#D3E0F0",
    accent: "#123E7A", accentBright: "#1D5AA8", accentDeep: "#0A2C57",
    teamB: "#C9682F",
    faintAccent: "#9DB9DC", faintAccent2: "#8CA9CF",
    headFrom: "#1D5AA8", headTo: "#123E7A", headBFrom: "#B4571F", headBTo: "#8C4014", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#1D5AA8", profFair: "#C99A1E", profWeak: "#9AA3AD",
  },
  dark: {
    label: "ダーク",
    bgFrom: "#1B222C", bgVia: "#171D26", bgTo: "#12161D",
    text: "#E8EDF2", textSub: "#A9B4C0", textFaint: "#6E7A87",
    surface: "#232B36", surfaceAlt: "#1E252F", surfaceWhite: "#2A323D",
    border: "#39434F", borderInput: "#465162", borderTable: "#2E3742",
    accent: "#4FA8E0", accentBright: "#6BC1F5", accentDeep: "#2E6FA0",
    teamB: "#E2793F",
    faintAccent: "#3A4653", faintAccent2: "#333E4A",
    headFrom: "#2E6FA0", headTo: "#1D4A73", headBFrom: "#A8542A", headBTo: "#7E3C1C", badgeBg: "#E8EDF2",
    profGreat: "#B571D9", profGood: "#6BC1F5", profFair: "#E0C15C", profWeak: "#7C8894",
  },
  sakura: {
    label: "桜",
    bgFrom: "#FAEFF3", bgVia: "#F4DCE5", bgTo: "#EEC7D6",
    text: "#4E0D28", textSub: "#903E5E", textFaint: "#BF7C97",
    surface: "#FFFFFF", surfaceAlt: "#FBEFF4", surfaceWhite: "#FFFBFD",
    border: "#E2B0C4", borderInput: "#D997B1", borderTable: "#F0D3DF",
    accent: "#8F1248", accentBright: "#B82064", accentDeep: "#6B0A34",
    teamB: "#0F6E86",
    faintAccent: "#DC9DB6", faintAccent2: "#CF8CA7",
    headFrom: "#B82064", headTo: "#8F1248", headBFrom: "#0F6E86", headBTo: "#0A5265", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#B82064", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  mint: {
    label: "青磁",
    bgFrom: "#EAF9F5", bgVia: "#D5F2EA", bgTo: "#BEEADD",
    text: "#073A32", textSub: "#2F7F71", textFaint: "#6FB2A6",
    surface: "#FFFFFF", surfaceAlt: "#EDFAF6", surfaceWhite: "#FAFFFD",
    border: "#A5DED1", borderInput: "#8BD3C2", borderTable: "#CBEEE3",
    accent: "#067060", accentBright: "#0F9682", accentDeep: "#045246",
    teamB: "#BE5A22",
    faintAccent: "#96D8C9", faintAccent2: "#85CBBB",
    headFrom: "#0F9682", headTo: "#067060", headBFrom: "#A84E1C", headBTo: "#813A12", badgeBg: "transparent",
    profGreat: "#8A3FA0", profGood: "#0F9682", profFair: "#A67F14", profWeak: "#9AA3AD",
  },
  sumi: {
    label: "墨",
    bgFrom: "#F5F5F2", bgVia: "#EBEBE6", bgTo: "#DEDED6",
    text: "#1E1E1C", textSub: "#55554F", textFaint: "#8C8C83",
    surface: "#FFFFFF", surfaceAlt: "#F4F4F0", surfaceWhite: "#FCFCFA",
    border: "#CCCCC2", borderInput: "#BABAAF", borderTable: "#E0E0D8",
    accent: "#2A2A28", accentBright: "#474743", accentDeep: "#171716",
    teamB: "#A81E0A",
    faintAccent: "#C4C4BA", faintAccent2: "#B7B7AC",
    headFrom: "#3A3A38", headTo: "#242422", headBFrom: "#9A1B09", headBTo: "#761505", badgeBg: "transparent",
    profGreat: "#6E4A85", profGood: "#3E6690", profFair: "#8A6C0C", profWeak: "#9AA3AD",
  },
  midnight: {
    label: "宵闇（ダーク紺）",
    bgFrom: "#161B28", bgVia: "#121724", bgTo: "#0D111B",
    text: "#E5EAF4", textSub: "#A2AECB", textFaint: "#66738F",
    surface: "#1D2434", surfaceAlt: "#181F2D", surfaceWhite: "#242C3F",
    border: "#333F58", borderInput: "#3F4D6C", borderTable: "#293349",
    accent: "#7C9EDB", accentBright: "#9AB8EC", accentDeep: "#4C6BA6",
    teamB: "#E08A52",
    faintAccent: "#37425C", faintAccent2: "#303A52",
    headFrom: "#3D5A8F", headTo: "#28406B", headBFrom: "#A0562E", headBTo: "#7A3F1F", badgeBg: "#E5EAF4",
    profGreat: "#BC85DD", profGood: "#9AB8EC", profFair: "#DFC26A", profWeak: "#7C8894",
  },
  nibi: {
    label: "鈍色（低刺激ライト）",
    bgFrom: "#E9EBEC", bgVia: "#E2E5E7", bgTo: "#D8DCDF",
    text: "#33404A", textSub: "#68757F", textFaint: "#98A3AB",
    surface: "#F0F2F3", surfaceAlt: "#E8EBEC", surfaceWhite: "#F3F5F6",
    border: "#C5CCD1", borderInput: "#B4BDC4", borderTable: "#D5DADD",
    accent: "#35566F", accentBright: "#4A7290", accentDeep: "#284257",
    teamB: "#A3653E",
    faintAccent: "#BCC7CE", faintAccent2: "#AFBBC3",
    headFrom: "#4E7089", headTo: "#3A566C", headBFrom: "#8F5636", headBTo: "#704226", badgeBg: "transparent",
    profGreat: "#7E5E93", profGood: "#5F829B", profFair: "#9C8351", profWeak: "#9AA3AD",
  },
  kinari: {
    label: "生成り（紙）",
    bgFrom: "#EEEAE0", bgVia: "#E9E4D8", bgTo: "#E0DACB",
    text: "#453E32", textSub: "#77705F", textFaint: "#A39B87",
    surface: "#F3F0E7", surfaceAlt: "#ECE8DC", surfaceWhite: "#F6F3EB",
    border: "#D2CBB9", borderInput: "#C4BCA7", borderTable: "#DED8C9",
    accent: "#5E5138", accentBright: "#7A6B4B", accentDeep: "#463C28",
    teamB: "#8F5340",
    faintAccent: "#CCC4AF", faintAccent2: "#C0B7A0",
    headFrom: "#70634A", headTo: "#544935", headBFrom: "#7C4736", headBTo: "#623527", badgeBg: "transparent",
    profGreat: "#7E5E93", profGood: "#6E7F5A", profFair: "#9C7B45", profWeak: "#9F988A",
  },
  susutake: {
    label: "煤竹（ダーク茶）",
    bgFrom: "#26211D", bgVia: "#211C19", bgTo: "#1A1613",
    text: "#E6E0D8", textSub: "#B2A99D", textFaint: "#786F64",
    surface: "#2E2823", surfaceAlt: "#28221E", surfaceWhite: "#362F29",
    border: "#453D35", borderInput: "#544A40", borderTable: "#39322B",
    accent: "#C3A26E", accentBright: "#D6B885", accentDeep: "#96794B",
    teamB: "#C77A55",
    faintAccent: "#463E34", faintAccent2: "#3E362D",
    headFrom: "#7A6444", headTo: "#5C4A31", headBFrom: "#8F5638", headBTo: "#6E3F27", badgeBg: "#E6E0D8",
    profGreat: "#B08BC6", profGood: "#D6B885", profFair: "#C9B25C", profWeak: "#8A8178",
  },
  koke: {
    label: "苔（ダーク緑）",
    bgFrom: "#1C231E", bgVia: "#171E19", bgTo: "#121813",
    text: "#DFE6DC", textSub: "#A4B1A0", textFaint: "#697566",
    surface: "#232C25", surfaceAlt: "#1E2620", surfaceWhite: "#2A342C",
    border: "#38453A", borderInput: "#445446", borderTable: "#2D392F",
    accent: "#8CAF85", accentBright: "#A3C69B", accentDeep: "#63855D",
    teamB: "#C08857",
    faintAccent: "#39463B", faintAccent2: "#323E34",
    headFrom: "#4E6B4A", headTo: "#3A5237", headBFrom: "#8F5F38", headBTo: "#6E4728", badgeBg: "#DFE6DC",
    profGreat: "#A98FC4", profGood: "#A3C69B", profFair: "#CCB964", profWeak: "#7E877B",
  },
};

// ---- 書体プリセット ----------------------------------------------------
export const FONT_PRESETS = {
  kyokasho: { label: "教科書体（デフォルト）", stack: "'UD Digi Kyokasho NK-B','UD デジタル 教科書体 NK-B','Zen Maru Gothic',sans-serif" },
  gothic: { label: "ゴシック体", stack: "'Noto Sans JP','Hiragino Kaku Gothic ProN','Yu Gothic',sans-serif" },
  mincho: { label: "明朝体", stack: "'Zen Old Mincho','Hiragino Mincho ProN','Yu Mincho',serif" },
  gyousho: { label: "行書", stack: "'Yuji Syuku','Zen Kurenaido',serif" },
  rounded: { label: "丸文字", stack: "'Klee One','Zen Maru Gothic',sans-serif" },
};

export const THEME_LIST = Object.keys(THEME_PRESETS).map((key) => ({ key, label: THEME_PRESETS[key].label }));
export const FONT_LIST = Object.keys(FONT_PRESETS).map((key) => ({ key, label: FONT_PRESETS[key].label }));

// コンポーネント側が参照する theme オブジェクト: 各値はCSS変数参照文字列
const COLOR_KEYS = Object.keys(THEME_PRESETS.sky);
export const theme = {};
COLOR_KEYS.forEach((k) => { if (k !== "label") theme[k] = `var(--cs-${k})`; });

// 実際にCSS変数へ値を書き込む(テーマ切替時・初期化時に呼ぶ)
export function applyTheme(colorKey, fontKey) {
  const c = THEME_PRESETS[colorKey] || THEME_PRESETS.sky;
  const f = FONT_PRESETS[fontKey] || FONT_PRESETS.kyokasho;
  const root = document.documentElement.style;
  COLOR_KEYS.forEach((k) => { if (k !== "label") root.setProperty(`--cs-${k}`, c[k]); });
  root.setProperty("--cs-font", f.stack);
}

export const WIN_BADGE_IMG = "data:image/png;base64,";
export const LOSE_BADGE_IMG = "data:image/png;base64,";
